...
const FUZZY_STRATEGY = {
  IGNORE_CASE: 'ignoreCase',
  IGNORE_SPACE_PUNCTUATION: 'ignoreSpacePunctuation',
  LEVENSHTEIN: 'levenshtein',
};

const LEVENSHTEIN_THRESHOLD_RANGE = { min: 1, max: 3 };
 ...
 */
function fuzzyMatchKey(keyValue, candidates, fuzzyConfig) {
 ...
 */
function buildCandidateList(fileAData, keyColumnA) {
  const seen = new Map();
 ...
 * @returns {Object} 处理后的新行
 */
function matchRowWithRule(rowB, rowIndex, fileAMap, candidates, keyColumnB, selectedColumns, stats, fuzzyConfig) {
  const normalizedRowB = {};
  for (const key of Object.keys(rowB)) {
    normalizedRowB[key] = normalizeValue(rowB[key]);
  }
 ...
 * @returns {Object} 处理后的新行
 */
function matchRowWithRuleChain(rowB, rowIndex, ruleContexts, selectedColumns, stats) {
  const normalizedRowB = {};
  for (const key of Object.keys(rowB)) {
    normalizedRowB[key] = normalizeValue(rowB[key]);
  }
